...
if (file.exists("results/chip-subtypes/sf3b1_chip.txt")) {
    file.remove("results/chip-subtypes/sf3b1_chip.txt")
}
...
out_raw <- fread("data/kessler_sf3b1_chip.tsv.gz") 
...
p1 <- p1 + 
    theme_bw() + 
    ylab(expression(paste("SNP effect on ", italic("SF3B1"), "CHIP"))) +
...
p2 <- mr_forest_plot(res_single)[[1]] +
    theme_bw() +
    xlab(expression(paste("MR effect size for telomere length on ", italic("SF3B1"), "CHIP"))) +
    theme(
        text = element_text(family = "alegreya_sans", size = 75),
        legend.position = "none"
    ) 

ggsave("plots/chip-subtypes/sf3b1_chip_scatter.png", p1, width = 9, height = 6, dpi = 600)
ggsave("plots/chip-subtypes/sf3b1_chip_loo.png", p2, width = 6, height = 6, dpi = 600)

cat("MR Analysis of telomere length on SF3B1 CHIP\n", file = "results/chip-subtypes/sf3b1_chip.txt", append = TRUE)
...
capture.output(print(mr_res), file = "results/chip-subtypes/sf3b1_chip.txt", append = TRUE)

cat("\nMR Egger Pleiotropy Test:\n", file = "results/chip-subtypes/sf3b1_chip.txt", append = TRUE)
capture.output(print(res_pleio), file = "results/chip-subtypes/sf3b1_chip.txt", append = TRUE)

cat("\nCochran's Q:\n", file = "results/chip-subtypes/sf3b1_chip.txt", append = TRUE)
capture.output(print(res_het), file = "results/chip-subtypes/sf3b1_chip.txt", append = TRUE)

cat("\nInstrument F Statistics:\n", file = "results/chip-subtypes/sf3b1_chip.txt", append = TRUE)
capture.output(print(instrument_f_statistics), file = "results/chip-subtypes/sf3b1_chip.txt", append = TRUE)
