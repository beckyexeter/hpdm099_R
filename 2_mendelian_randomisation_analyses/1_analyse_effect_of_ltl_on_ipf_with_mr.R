...
if (file.exists("results/ltl_ipf_mr_results.txt")) {
    file.remove("results/ltl_ipf_mr_results.txt")
}
...
out_raw <- fread("data/gbmi_eur_ipf.tsv.gz") 
...
p1 <- p1 + 
    theme_bw() + 
    ylab("SNP effect on IPF") +
    xlab("SNP effect on telomere length") +
...
p2 <- mr_forest_plot(res_single)[[1]] +
    theme_bw() +
    xlab("MR effect size for telomere length on IPF") +
    theme(
        text = element_text(family = "alegreya_sans", size = 75),
        legend.position = "none"
    ) 

ggsave("plots/ltl_ipf_mr_scatter_plot.png", p1, width = 9, height = 6, dpi = 600)
ggsave("plots/ltl_ipf_mr_leave_one_out_plot.png", p2, width = 6, height = 6, dpi = 600)

cat("MR Analysis of telomere length on IPF\n", file = "results/ltl_ipf_mr_results.txt", append = TRUE)
...
cat("\ntelomere length on IPF MR Results:\n", file = "results/ltl_ipf_mr_results.txt", append = TRUE)
capture.output(print(mr_res), file = "results/ltl_ipf_mr_results.txt", append = TRUE)

cat("\nMR Egger Pleiotropy Test:\n", file = "results/ltl_ipf_mr_results.txt", append = TRUE)
capture.output(print(res_pleio), file = "results/ltl_ipf_mr_results.txt", append = TRUE)

cat("\nCochran's Q:\n", file = "results/ltl_ipf_mr_results.txt", append = TRUE)
capture.output(print(res_het), file = "results/ltl_ipf_mr_results.txt", append = TRUE)

cat("\nInstrument F Statistics:\n", file = "results/ltl_ipf_mr_results.txt", append = TRUE)
capture.output(print(instrument_f_statistics), file = "results/ltl_ipf_mr_results.txt", append = TRUE)
